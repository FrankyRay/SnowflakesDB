export default function parseCoordinate(player, input) {
    const locOffset = input.split(" ").map((v) => v.trim());
    //! Too many value
 ...
 */
function worldCoordinate(player, offset) {
    const newVector = {};
    const axis = ["x", "y", "z"];
 ...
 * by `WavePlayz#7915`
 */
function localCoordinate(player, offset) {
    const norm = ({ x, y, z }, s) => {
